...
Your Astro server runs separately from Payload, so draft access needs an explicit strategy. The pattern used here and in the [example](./example):
...
The [`example/`](./example) directory contains a complete working setup: a Payload app (`example/payload`) and an Astro site (`example/website`) sharing a MongoDB database via the Local API.
